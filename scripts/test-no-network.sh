...
# Just verify the marker file still exists and status shows enabled
if [ -f ~/.calf-no-network ]; then
    pass "Network isolation marker file still exists"
else
    fail "Network isolation marker file missing"
fi

output=$(./scripts/calf-bootstrap --status 2>&1)
if echo "$output" | grep -q "Network: Isolated"; then
    pass "Status still shows network isolation enabled"
else
    fail "Status does not show network isolation"
    echo "Output: $output"
fi
